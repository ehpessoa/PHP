<?php	
    ...
    if ( strpos($phones, ",") == 0 ) {
		...
		if ( endsWith($phones, '.csv') ) { 			
			...
			$groupfile = fopen($group, "r") or die("Unable to open file ". $phone);		
	...
	if ( !empty($message) ) {
		$w->sendBroadcastMessage($arrayphones, $message);
		while($w->pollMessage());
		sleep(3);
	}
	if ( !empty($image) ) { 
		$w->sendBroadcastImage($arrayphones, $path . '/files/' . $_SESSION['zap']['username'] . '/' . $image);
		while($w->pollMessage());
		sleep(3);
	}
	if ( !empty($audio) ) {			
		$w->sendBroadcastAudio($arrayphones, $path . '/files/' . $_SESSION['zap']['username'] . '/' . $audio);
		while($w->pollMessage());
		sleep(3);
	}
	if ( !empty($video) ) { 
		$w->sendBroadcastVideo($arrayphones, $path . '/files/' . $_SESSION['zap']['username'] . '/' . $video);		
		while($w->pollMessage());
		sleep(3);
	}	
